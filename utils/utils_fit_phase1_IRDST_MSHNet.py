...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if (epoch + 1) >=20:
        ...
        with open(input_file, 'r') as f_input:
            ...
            for line in input_lines:
                ...
                if len(parts) > 0:
                    ...
                    orig_img_path = aug_img_path.replace("IRDST-M_AUG", "IRDST-M")
        ...
        update_pseudo_labels(input_file, candidate_file, epoch+1, specified_epoch=20, iou_threshold=0.1)
        ...
        if len(predictions) > 0:
            for prediction in predictions:
                if len(prediction) >= 4:
                    ...
                    print(f"Warning: The prediction box format is incorrect and has been skipped. Prediction: {prediction}")
        else:
